...
> ⚠️ **IS/OS 铁律（ABSOLUTE）**：OS（样本外）仅用于验证 IS 优化结果的泛化稳定性，严禁将 OS 作为优化目标。对 OS 指标做任何形式的调参/筛选/回溯均为违规。详见 [`docs/is_os_discipline.md`](is_os_discipline.md)。
>
> 核心规则：
> - 策略选择只能基于 IS 指标，OS 仅做「一票否决」
> - 看到 OS 结果后回溯修改策略参数 = 违规
> - 按 OS 指标排序/筛选多个策略候选 = 违规
> - OS 验证失败 → 标记失败，不做参数回溯
